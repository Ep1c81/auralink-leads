...
export interface NicheSweepResult {
  niche: string;
  found: number;
  imported: number;
  skipped: number;
...
export interface AutoProspectSweepResult {
  ranAt: string;
  location: string;
  results: NicheSweepResult[];
}
 ...
 * WhatsApp pitch. Shared by the dashboard-triggered /api/cron/auto-prospect
 * route (no auth — same-origin only) and the Vercel-Cron-triggered
 * /api/cron/sweep route (CRON_SECRET-gated), so both entry points run the
 * exact same tested logic.
 */
export async function runAutoProspectSweep(): Promise<AutoProspectSweepResult> {
